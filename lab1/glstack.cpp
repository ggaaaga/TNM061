...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        modelViewStack.rotX(static_cast<float>(-M_PI_2));  // Orient the poles along Y axis instead of Z AKA snurr glob så den rak
        ...
        // Planet 1
        modelViewStack.rotY(time);            //orbit rot
        modelViewStack.translate(1.0f, 0.0f, 0.0f);  

        modelViewStack.rotY(2.0 * time);  //axis rotation
        modelViewStack.scale(0.1f);        
        // Update the transformation matrix in the shader
        glUniformMatrix4fv(location_MV, 1, GL_FALSE, modelViewStack.getCurrentMatrix());
        ...
        // Planet 2
        modelViewStack.rotY(time);  // orbit rot
        modelViewStack.translate(2.0f, 0.0f, 0.0f);

        modelViewStack.rotY(2.0 * time);  // axis rotation
        modelViewStack.scale(0.2f);
        // Update the transformation matrix in the shader
        glUniformMatrix4fv(location_MV, 1, GL_FALSE, modelViewStack.getCurrentMatrix());
        // Render the geometry to draw the earth
        glBindTexture(GL_TEXTURE_2D, earthTexture.texID);
        ...
        //Moon
        modelViewStack.rotY(time);                      // Moon orbit rotation
        modelViewStack.translate(0.4f, 0.0f, 0.0f);     //Moon orbit radius
        //push?
        modelViewStack.rotY(15.0f * time);              //Moon rotation around axis
        modelViewStack.scale(0.1f);
