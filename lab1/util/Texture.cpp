...
Texture::Texture(const char* filename) { createTexture(filename); }
...
{                                                // (based on NeHe's TGA loading code)
    ...
    tga.imageSize =
        (tga.bytesPerPixel * tga.width * tga.height);  // Compute the total amount of memory needed
    ...
    if (fread(this->imageData, 1, tga.imageSize, TGAfile) !=
...
int Texture::loadTGA(const char* filename) {
    ...
    if (fread(&tgaheader, sizeof(TGAHeader), 1, TGAfile) ==
        0)  // Attempt to read 12 byte file header
    {
...
void Texture::createTexture(const char* filename) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, this->width, this->height, 0, this->type,
                 GL_UNSIGNED_BYTE, this->imageData);
